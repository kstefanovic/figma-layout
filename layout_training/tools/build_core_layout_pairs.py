"""Build directed source-to-target CORE layout pairs."""
...
from layout_training.pairs import build_core_pairs_with_stats, read_jsonl, write_jsonl


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build directed source-target CORE layout pairs from records.")
    ...
    pairs, stats = build_core_pairs_with_stats(records, families, min_matched_tokens=args.min_matched_tokens)
    write_jsonl(args.output, pairs)
    summary_path = Path(args.output).parent / "core_pairs_summary.json"
