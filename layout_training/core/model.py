"""Transformer model for simplified CORE top-level layout prediction."""
...
def _torch_modules():
    try:
        ...
        raise RuntimeError("PyTorch is required for CoreTopLevelLayoutTransformer.") from exc
    return torch, nn, F
...
class CoreTopLevelLayoutTransformer(nn.Module):
    def __init__(
        self,
        *,
        num_roles: int,
        ...
        self.input_proj = nn.Linear((d_model // 2) + (d_model // 4), d_model)
    ...
    def forward(self, x_num, role_ids, mask, source_center):
        role = self.role_embedding(role_ids)
        num = self.numeric_proj(x_num)
        x = self.input_proj(torch.cat([num, role], dim=-1))
